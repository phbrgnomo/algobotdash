...
O dashboard local está implementado com FastAPI, SQLite e Docker Compose. Ele importa o workbook, publica a projeção de modo atômico e oferece filtros, tabela de posições, métricas e risco. As próximas evoluções permanecem nas [issues do projeto](https://github.com/phbrgnomo/algobotdash/issues).

## Objetivo

O `algobotdash` é uma aplicação local que recebe o relatório padrão de trades exportado pelo MetaTrader 5 e:
...
- oferece filtros, tabela, métricas e cartões de risco sincronizados;
...
- **Fuso analítico:** identificador IANA obrigatório que define a interpretação das datas em toda a análise.
...
A mesma resposta acrescenta `sharpe_daily`, `sortino_daily`, `sharpe_annualized` e `sortino_annualized`, além do período efetivo, quantidade de dias úteis e cobertura auditável do saldo de abertura ajustado. Os motivos desses quatro índices ficam separadamente em `temporal_unavailable_reasons`, para não serem tratados como falhas das métricas por posição. O retorno diário é o P&L das posições filtradas encerradas no dia dividido pelo saldo global de abertura ajustado; dias úteis sem encerramentos recebem retorno zero e fins de semana são excluídos. O ajuste é identificado pelo comentário exato `Ajuste de Saldo`. O primeiro ajuste do dia no fuso analítico define a referência; seu saldo é reduzido pelo P&L, comissão, taxa e swap de transações globais anteriores. Ajustes posteriores não redefinem o dia.
...
`duration_days`. Timestamps são UTC; o dashboard mostra datas no fuso devolvido por `/api/status`.
...
fechamento selecionado daquele dia no fuso analítico; dias úteis sem fechamentos não alteram
...
O runtime padrão é Docker Compose, com o dashboard disponível em `http://localhost:8765`.
...
A configuração YAML é a fonte canônica dos agrupamentos e do fuso analítico. O dashboard não edita o YAML.

`timezone` é obrigatório no topo do YAML e deve ser um identificador IANA não vazio. `America/Bahia` é o valor recomendado e usado no exemplo:

```yaml
timezone: America/Bahia
source:
  path: source/ReportHistory.xlsx
```

O fuso define a interpretação de timestamps sem offset no Excel, filtros por data, saldos de abertura, métricas temporais, drawdowns e a renderização das datas. Alterá-lo torna a projeção atual indisponível até uma nova reconstrução do workbook; isso evita analisar timestamps existentes sob uma convenção diferente.
